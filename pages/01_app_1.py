...
# ==========================================
    ...
    if ensemble_model_pkg is not None:
        ...
        # [수정 반영] 사이드바에서 받아온 window_size를 활용하여 실제 측정치(Ground Truth)에도 이동평균(Rolling) 적용
        rolling_window = window_size * 6 
        df_val = df_val.sort_values("시간").set_index("시간")
        df_val['실제 측정치 (Ground Truth)'] = df_val['실제 측정치 (Ground Truth)'].rolling(window=rolling_window, min_periods=1).mean()
