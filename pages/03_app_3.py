...
.mini-resource-card span {color:#91A4B7 !important;font-size:.73rem;font-weight:700;}
.mini-resource-card b {color:#F4F8FC !important;font-size:1.02rem;font-weight:800;}
...
/* Focused dark-theme override for native inputs.
   Important: style only the actual control internals, not the outer Streamlit block,
   otherwise label areas become large blue rectangles. */
...
[data-testid="stSelectbox"] [data-baseweb="select"]:focus-within > div {
  border-color:#5EA9EE !important;
  box-shadow:0 0 0 1px rgba(94,169,238,.22) inset !important;
}
/* Number-input step buttons are separate from the input surface in Streamlit 1.37. */
...
/* Keep text-area outline subtle instead of bright white. */
[data-testid="stTextArea"] textarea {
  outline:none !important;
  border:1px solid #2D4661 !important;
  border-radius:8px !important;
}
[data-testid="stTextArea"] textarea:focus {
  border-color:#5EA9EE !important;
  box-shadow:0 0 0 1px rgba(94,169,238,.18) !important;
}
...
with tab_scenario:
    ...
    # Streamlit 1.37의 data_editor는 내부 Canvas를 사용해 일부 테마에서
    # 밝은 흰색 셀/헤더를 강제로 표시합니다. 따라서 편집은 다크 슬라이더로,
    # 전체 현황은 순수 HTML 다크 표로 분리해 렌더링합니다.
    def render_resource_group_editor(group_name: str, areas: list[str], group_key: str) -> None:
        group_df = editor_df[editor_df["구역"].isin(areas)].reset_index(drop=True).copy()
        selected_resource = st.selectbox(
            "조정 대상",
            areas,
            key=f"resource_select_{group_key}_{state_key}",
        )
        selected_row = group_df[group_df["구역"] == selected_resource].iloc[0]
        current_value = int(scenario_units[selected_resource])
        max_value = int(MAX_UNITS[selected_resource])

        info1, info2, info3, info4 = st.columns(4)
        with info1:
            st.markdown(
                f'<div class="mini-resource-card"><span>현재 인원</span><b>{float(selected_row["현재 인원"]):.1f}명</b></div>',
                unsafe_allow_html=True,
            )
        with info2:
            st.markdown(
                f'<div class="mini-resource-card"><span>계획 운영</span><b>{int(selected_row["계획 운영"])}개</b></div>',
                unsafe_allow_html=True,
            )
        with info3:
            st.markdown(
                f'<div class="mini-resource-card"><span>기준 권고</span><b>{int(selected_row["기준 권고"])}개</b></div>',
                unsafe_allow_html=True,
            )
        with info4:
            st.markdown(
                f'<div class="mini-resource-card"><span>시나리오 직원</span><b>{staff_from_units(selected_resource, current_value)}명</b></div>',
                unsafe_allow_html=True,
            )

        control_key = f"resource_slider_{group_key}_{state_key}_{selected_resource}_{current_value}"
        edited_value = st.slider(
            f"{selected_resource} 시나리오 운영 수",
            min_value=0,
            max_value=max_value,
            value=current_value,
            step=1,
        ...
        st.markdown(
            f'<div class="slider-range-labels"><span>0</span><span>{max_value}</span></div>',
            unsafe_allow_html=True,
        )

        if int(edited_value) != current_value:
            value = clamp_units(selected_resource, int(edited_value))
            st.session_state["scenario_units"][selected_resource] = value
            if selected_resource == focus_line:
                st.session_state["pending_focus_slider"] = (slider_key, value)
            st.rerun()

        display_df = group_df.copy()
        display_df["시나리오 운영"] = display_df["구역"].map(
            lambda area: int(st.session_state["scenario_units"].get(area, scenario_units[area]))
        )
        display_df["시나리오 직원"] = display_df.apply(
            lambda row: staff_from_units(str(row["구역"]), int(row["시나리오 운영"])),
            axis=1,
        )
        render_dark_table(display_df, max_height=420)

    edit_tab1, edit_tab2, edit_tab3 = st.tabs(["유인 체크인 라인", "셀프 체크인 기기", "IM1·IM2 출입문"])
    with edit_tab1:
        render_resource_group_editor("유인 체크인 라인", STAFFED_AREAS, "staffed")
    with edit_tab2:
        render_resource_group_editor("셀프 체크인 기기", SELF_AREAS, "self")
    with edit_tab3:
        render_resource_group_editor("IM1·IM2 출입문", IM_AREAS, "im")
...
with tab_opt:
    ...
    with oc2:
        additional_staff = st.slider("추가 투입 가능 인력", min_value=0, max_value=50, value=4, step=1)
        st.markdown('<div class="slider-range-labels"><span>0</span><span>50</span></div>', unsafe_allow_html=True)
    with oc3:
