...
with tab_overview:
    ...
    with right:
        ...
        st.dataframe(dark_table(show), use_container_width=True, hide_index=True)

        st.markdown('<div class="section-title">향후 항공편 수요</div>', unsafe_allow_html=True)
        st.dataframe(dark_table(active_flights.head(10)), use_container_width=True, hide_index=True)
...
with tab_scenario:
    ...
    if selected_airline == "적용 안 함" or demand_change_pct == 0:
        ...
        if not airline_detail.empty:
            st.dataframe(
                dark_table(airline_detail.sort_values("출발기준시각").head(20)),
                use_container_width=True,
                hide_index=True,
            )
...
with tab_result:
    ...
    st.dataframe(dark_table(bott), use_container_width=True, hide_index=True)
    ...
    if mc is not None and st.session_state.get("mc_signature") == current_sig:
        ...
        st.dataframe(dark_table(summary), use_container_width=True, hide_index=True)
...
with tab_opt:
    ...
    if opt and opt.get("state_key") == state_key and opt.get("airline") == selected_airline and opt.get("shock") == demand_change_pct:
        ...
        if changes:
            st.dataframe(dark_table(pd.DataFrame(changes).sort_values("변화", key=lambda s: s.abs(), ascending=False)), use_container_width=True, hide_index=True)
        else:
...
with tab_quality:
    ...
    st.dataframe(dark_table(checks), use_container_width=True, hide_index=True)
...
"""
    ...
    st.dataframe(dark_table(rules), use_container_width=True, hide_index=True)

    with st.expander("현재 선택 시각 원본 스냅샷 보기"):
        st.dataframe(dark_table(snap), use_container_width=True, hide_index=True)
